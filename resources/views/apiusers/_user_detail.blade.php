...
<div class="box-body">
  ...
  <table id="example2" class="table table-bordered table-hover">
    <tbody>
        <tr>
            <td>User name</td>
            <td><input type="text" name="" value="{{$user_detail->name}}">
            </td>
        </tr>
         <tr>
            <td>Email</td>
            <td><input type="email" name="" value="{{$user_detail->email}}">
            </td>
        </tr>
       @foreach($user_detail->meta as  $value )
        ...
        @elseif($value->key == 'designation')
            @if(App\Designation::getDesignation($value->value)!=false)
                <td>{{$value->key}}</td>
                <td><input type="text" name="" value="{{App\Designation::getDesignation($value->value)}}"></td>
             @endif                  
        @elseif($value->key == 'ministry')
            ...
            @if(App\Ministrie::ministryName($json[0])!=false)
                <td>{{$value->key}}</td>
                 <td>
                   <input type="text" name="" value="{{App\Ministrie::ministryName($json[0])}}">
                 </td>
            @endif
            ...
            <?php $dep = json_decode($value->value);  ?>
                @if(App\Department::getDepName($dep[0])!=false) 
                    ...
                    <td>
                        ...
                        <input type="text" name="" value="{{App\Department::getDepName($dep[0])}}">
                    </td>
        @endif
        @else
         <td>{{$value->key}}</td>
